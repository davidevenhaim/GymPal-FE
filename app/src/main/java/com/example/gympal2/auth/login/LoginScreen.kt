...
fun LoginScreen(
    ...
    val screenState by loginStateHolder.state.collectAsState()
    ...
    Column(
        ...
        Button(onClick = { screenState.onLoginClicked() }) {
            Text("Login")
        }

        when (authSate) {
            is AuthState.Error -> ErrorText(text = "Error while login ${(authSate as AuthState.Error).message}")
